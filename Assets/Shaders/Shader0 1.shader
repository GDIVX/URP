Shader "Unlit/Shader1" // shader name  - shader1 under shader category - Unlit
{
    ...
    Properties
    {
        ...
        _Power("power", Float) = 2 //with a specified range.
    ...
    // specific shader (there might be more then one subshader) under one defined shader.
    SubShader
    {
        Tags
        {
            "RenderType"="Opaque"
        } // common tags: Opaque, Transparent, Overlay,
        LOD 100
        ...
        Pass
        {
            ...
            v2f vert(appdata v)
            {
                ...
                o.viewDirection = normalize(_WorldSpaceCameraPos - world_pos);
                return o;
            }
            ...
            {
                float3 color = dot(i.normal, i.viewDirection);
                float4 finalColor = float4(color, 1);
                finalColor = 1 - saturate(finalColor);
                finalColor = pow(finalColor , _Power);
                return finalColor;
            }
            ENDCG
        }
